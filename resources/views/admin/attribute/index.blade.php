        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="mt-2 float-left">Attributes</h4>
                    <a class="btn btn-info btn-sm float-right" href="{{ route('admin.attributes.create') }}"><i class="mdi mdi-plus-circle-outline"></i> Create Attributes</a>
                    <button class="btn-delete btn btn-danger btn-sm float-right mr-2" data-url="{{ route('admin.attribute.destroy') }}" disabled=""><i class="mdi mdi-delete"></i> Delete</button>
                </div>
                <div class="card-body">
...
@push('js')
    ...
    <script>
        $(function() {
            $('#datatable').DataTable({
                serverSide: true,
                ajax: "{{ route('admin.attributes.datatables') }}",
                columns: [
                    { name: 'name' },
                    { name: 'status' },
