        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="mt-2 float-left">Coupons</h4>
                    <a class="btn btn-info btn-sm float-right" href="{{ route('admin.coupons.create') }}"><i class="mdi mdi-plus-circle-outline"></i> Create Coupon</a>
                    <button class="btn-delete btn btn-danger btn-sm float-right mr-2" data-url="{{ route('admin.coupon.destroy') }}" disabled=""><i class="mdi mdi-delete"></i> Delete</button>
                </div>
                <div class="card-body">
...
@push('js')
    ...
    <script>
        $(function() {
            $('#datatable').DataTable({
                serverSide: true,
                ajax: "{{ route('admin.coupons.datatables') }}",
                columns: [
                    { name: 'name'},
                    { name: 'code'},
                    { name: 'value' },
